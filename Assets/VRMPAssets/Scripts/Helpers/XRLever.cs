...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRLever : XR.Interaction.Toolkit.Interactables.XRBaseInteractable
    {
        ...
        void Update()
        {
            allowX = rotationAxis.HasFlag(Axis.X);
            allowY = rotationAxis.HasFlag(Axis.Y);
            allowZ = rotationAxis.HasFlag(Axis.Z);
        }
        ...
        void UpdateValue()
        {
            ...
            SetHandleAngle(lookAngle, allowX, allowY, allowZ);
        ...
        void SetValue(bool isOn, bool forceRotation = false)
        {
            ...
            {
                if (forceRotation)
                    SetHandleAngle(m_Value ? m_MaxAngle : m_MinAngle, allowX, allowY, allowZ);
        ...
        void SetHandleAngle(float angle, bool allowX, bool allowY, bool allowZ)
        {
            float xAngle = m_Handle.localEulerAngles.x;
            float yAngle = m_Handle.localEulerAngles.y;
            float zAngle = m_Handle.localEulerAngles.z;
            if (m_Handle != null)
            {
                if (allowX)
                    m_Handle.localRotation = Quaternion.Euler(angle, yAngle, zAngle);
                if (allowY)
                    m_Handle.localRotation = Quaternion.Euler(xAngle, angle, zAngle);
                if (allowZ)
                    m_Handle.localRotation = Quaternion.Euler(xAngle, yAngle, angle);
            }
        }
